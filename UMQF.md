...
- `Tc(e)` - Temporal coefficient, from 0 to 1: `Tc = effect duration / reference lifespan`, capped at 1. The reference lifespan is the full lifespan of the Average Reference Entity of the entity's kind (e.g., Human ≈ 80 years — the same standard as GCU). For death, effect duration is the victim's remaining expected lifespan: a victim with a full life ahead gives Tc ≈ 1.00; a victim with 4 years left gives Tc = 4/80 = 0.05. The reason: what an action destroys or protects is survival time, so the score follows how much time was at stake. The cap means no lifespan, however long or indefinite, counts for more than one reference lifetime — the same way the VSA cap bounds capability. Linear scale mapping:
...
- Origin and Measurement of Consent and Suffering: Suffering and positive feeling are the entity's evolved or engineered estimate of its own survival-delta: the state that drove the quickest escape from harm was tagged suffering; the state that drove approach to survival gains, pleasure. `Sc`, `ΔSc` and `Vc` therefore multiply `ΔOS` — they surface the entity's own survival reading that external estimates miss. Behaviours like fleeing or resisting could indicate non-consent. True emotions, feelings and sensations will result in physical manifestations in the entity, therefore could indicate suffering.
...
- Minimal Self-Awareness for Early-Life Forms: Assign a minimal, non-zero self-awareness value (e.g., 0.0001-0.0010) to early-life entities like fertilized eggs or seeds, ensuring that their biological complexity and potential are not treated as zero.
...
- VSA(Farmer):
  ...
  - **Total VSA = 0.58**
- Tc = 0.75 (lifelong capital loss, discounted for partial recovery; the two lost seasons are priced in Av)
