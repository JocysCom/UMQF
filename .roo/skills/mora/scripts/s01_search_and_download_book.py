...
Outputs:
- MORA/analysis/{document}/source-document.epub
- MORA/analysis/{document}/manifest.json (initial)

Usage:
  python .ai/skills/mora/scripts/s01_search_and_download_book.py --id 68730 --document exploration-team
  python .ai/skills/mora/scripts/s01_search_and_download_book.py --url https://www.gutenberg.org/ebooks/68730 --document exploration-team
  python .ai/skills/mora/scripts/s01_search_and_download_book.py --file "/path/to/book.epub" --document exploration-team
  python .ai/skills/mora/scripts/s01_search_and_download_book.py --search "Exploration Team" --document exploration-team
"""
...
def write_initial_manifest(document: str, title: str, url: str, source_type: str) -> None:
    manifest_path = os.path.join(WORKSPACE_ROOT, "MORA", "analysis", document, OUTPUT_MANIFEST)
    manifest = {
...
def main():
    ...
    out_dir = os.path.join(WORKSPACE_ROOT, "MORA", "analysis", args.document)
    ensure_dir(out_dir)
    ...
    print(f"\nSuccess! Document saved to MORA/analysis/{args.document}/{OUTPUT_EPUB}")

    # Create initial manifest
    write_initial_manifest(args.document, args.document, final_url, "book" if args.id else "web")

    print(f"\nNext Step: Extract metadata:")
    print(f"  python .ai/skills/mora/scripts/{NEXT_STEP_SCRIPT} --document {args.document}")
