---
name: mora
description: Run a MORA morality assessment on a document — analyze text (books, articles, transcripts) by identifying primary acting entities and scoring every significant action with the Universal Moral Quotient Formula (UMQF). Use this skill whenever the user asks to analyze a document for morality, evaluate entities with UMQF, run a MORA assessment, score actions by survival-impact (ΔOS, VSA, Tc, Vc, Sc, Rp, In), produce per-entity moral profiles (`{entity}.md` / `{entity}-actions.md`), or re-audit existing MORA entity files — even if they don't explicitly name "MORA" or "UMQF".
---
...
- Always read and refer to `UMQF.md` in the repository root for the complete UMQ formula, methodology, and examples.
...
### 1. Setup
- Use `python .ai/skills/mora/scripts/s01_search_and_download_book.py --search "{search}" --document {document}` to acquire and structure source text.
...
- **Verification**: Before analysis, read `MORA/analysis/{document}/manifest.json` to explicitly verify the Title and Author match your specific request. (e.g., Do not analyze Darwin if you asked for the Bible).
- Ensure the full text is saved to `MORA/analysis/{document}/source-document.md`.
- Read `UMQF.md` to refresh the formula and variables.

### 2. Analysis
- **Source Document Size Check**:
    - Run `python .ai/skills/mora/scripts/utils_check_size.py MORA/analysis/{document}/source-document.md`.
    - **Status: SMALL**: Read the entire file.
    - **Status: LARGE**:
        - **WARNING**: DO NOT read the full `source-document.md` directly. It will overflow context.
        - Read `MORA/analysis/{document}/segments.jsonl` to understand the structure.
        ...
        - **Iterative Processing**:
            - Use `python .ai/skills/mora/scripts/utils_extract_segment.py` to extract the specific range of lines (max ~10000 lines) to `MORA/analysis/{document}/temp_segment.md`.
...
### 3. Output Generation
For each main acting entity, create two files in `MORA/analysis/{document}/entities/`:
...
- **MANDATORY SECTIONS** (Strictly follow the structure in `.ai/skills/mora/references/entity.template.md`):
    - You MUST read and follow `.ai/skills/mora/references/entity.template.md` exactly. (This template overrides the output format in `UMQF.md`.)
...
- Template: `.ai/skills/mora/references/entity.template.md` — mandatory structure for `{entity}.md` output.
- Self-improvement prompt: `.ai/skills/mora/references/self-improvement.md` — use when the user asks to simplify, audit, or improve the MORA process itself.
- Python pipeline: `.ai/skills/mora/scripts/` — EPUB acquisition (`s01`), metadata extraction (`s02`), markdown conversion (`s03`), plus `utils_check_size.py` and `utils_extract_segment.py`.
- Dependencies: `.ai/skills/mora/requirements.txt` — install with `pip install -r .ai/skills/mora/requirements.txt` and `playwright install`.
